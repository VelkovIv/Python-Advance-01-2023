...
def player_choice(player):
    ...
    while choice < 0 or choice > columns:
        choice = int(input(f'Player {player}, please choose a column (1 - {columns}):\n'))
...
def check_win_condition(playground, row_index, column_index, win_counter):
    ...
    # indexes
    max_column_index = min(column_index + win_counter, len(playground[row_index]))
...
def check_draw_condition(playground):
...
def game_play_function(playground, win_counter):
    ...
    while True:
        ...
        if check_draw_condition(playground):
            print("Tha game is a Draw!")
            break
...
try:
    rows = int(input('Enter the number of rows (min 4): '))
    while rows < 4:
        rows = int(input('Enter the number of rows (min 4): '))
    columns = int(input(f'Enter the number of columns (min {rows + 1}): '))
    while columns < rows + 1:
        columns = int(input(f'Enter the number of columns (min {rows + 1}): '))
    print_function(create_matrix(rows, columns))
    win_counter = int(input(f'Enter the number of win combination (2 - {rows}) higher is more difficult: '))
    while win_counter < 2 or win_counter > rows:
        win_counter = int(input(f'Enter the number of win combination (2 - {rows}) higher is more difficult: '))
